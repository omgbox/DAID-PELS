...
def create_pipeline(config: dict = None, db_manager=None) -> Pipeline:
    ...
    Returns:
        Configured Pipeline
    """
    pipeline = Pipeline(config, db_manager=db_manager)
    ...
    pipeline.register_module('answer_engine', AnswerEngine(config, db_manager=db_manager))
...
def train(book_path: str, dict_path: str, db_path: str, max_passes: int = None):
    ...
    if not book_path_obj.exists():
        # Try relative to bookbot directory
        from .config import PROJECT_ROOT
        alt_path = PROJECT_ROOT / book_path
        if alt_path.exists():
            book_path = str(alt_path)
        else:
            print(f"\n  ERROR: Book not found: {book_path}")
            print(f"  Place your book in the books/ directory or use the default:")
            print(f"    python -m bookbot.main train")
            print(f"  Or download assets first:")
            print(f"    python download_assets.py")
            db.disconnect()
            return
    ...
    # Create pipeline with database already passed to constructor
    config = {}
    if max_passes:
        config['convergence'] = {'max_passes': max_passes}

    pipeline = create_pipeline(config, db_manager=db)
...
def query(db_path: str, single_query: str = None):
    ...
    # Create pipeline with db connection flowing through constructor
    pipeline = create_pipeline(db_manager=db)
    ...
    # Load SVO triples
    svo_rows = db.execute("SELECT subject, verb, object, sentence_id, confidence FROM svo_triples")
    context.svo_triples = [
        {
            'subject': r[0],
            'verb': r[1],
...
def _print_response(result: dict):
    """Print formatted response with multiple options if available."""
