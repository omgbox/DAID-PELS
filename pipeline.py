...
class Pipeline:
    ...
    def run_query(self, query: str, context: PipelineContext,
        ...
        """
        Run the query pipeline with conversation awareness.

        Args:
            query: User query string.
            context: PipelineContext from training.
        ...
        # Step 1: Contextual Query Rewriting (if conversation memory available)
        ...
        if rewriter and conversation_memory:
            ...
            rewrite_result = rewriter.process({
                'query': query,
                'intent': 'EXPLANATORY',  # Will be updated below
            })
        
        effective_query = rewrite_result['rewritten_query']
        self.logger.info(f"Effective query: {effective_query}")
        ...
        # Step 3: Trilateral BM25 Retrieval
        bm25 = self.modules.get('trilateral_bm25')
        if not bm25:
            bm25 = self.modules.get('bm25_engine')
        
        retrieval_results = []
        if bm25:
            # Combine effective query with expanded terms
        ...
        if conversation_memory:
            entities = self._extract_entities(effective_query)
