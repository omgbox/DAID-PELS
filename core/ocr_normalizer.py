...
# Try to import Pynini (disabled by default - use config to enable)
PYNINI_AVAILABLE = True  # Will be checked at runtime via try/except


class OCRNormalizer(BaseModule):
    """OCR artifact correction module."""

    # Character patterns that strongly suggest OCR corruption
    OCR_CONFUSION_SIGNALS = {
        'rn', 'cl', 'li', 'vv', 'ii', 'tli', 'vvv', 'rnr',
        'lrn', 'cli', 'rnc',
    }
    ...
    def _normalize_text(self, text: str) -> str:
        ...
        # Step 4: Correct OCR errors (smart auto-detection)
        if self.get_config('use_fst', True) and PYNINI_AVAILABLE:
            text = self._correct_with_pynini(text)
        elif self._needs_ocr_correction(text):
            self.logger.info("OCR corruption detected — running edit distance correction...")
            text = self._correct_with_edit_distance(text)
        else:
            self.logger.info("Text appears clean — skipping OCR correction")
    ...
    def _needs_ocr_correction(self, text: str) -> bool:
        """
        Dynamically detect if text has OCR corruption by sampling.
        Returns True if correction is likely needed.

        Checks two signals:
        1. High ratio of words unknown to the dictionary
           (clean fiction: ~2-5%; OCR-corrupted: >10%)
        2. Presence of character patterns typical of OCR confusion
           (e.g., 'rn' for 'm', 'vv' for 'w', 'tli' for 'cli')
        """
        words = text.split()
        sample_size = min(2000, len(words))
        ...
        for word in sample:
            ...
            total_clean += 1
            lower = clean.lower()
            if lower not in self.dictionary:
                unknown_count += 1
                if len(lower) >= 2:
                    for i in range(len(lower) - 1):
                        if lower[i:i+2] in self.OCR_CONFUSION_SIGNALS:
                            confusion_hits += 1
                            break

        unknown_ratio = unknown_count / total_clean if total_clean else 0
        confusion_ratio = confusion_hits / unknown_count if unknown_count else 0

        self.logger.info(
            f"OCR auto-detect: {unknown_count}/{total_clean} unknown "
            f"({unknown_ratio*100:.0f}%), "
            f"{confusion_hits} confusion signals ({confusion_ratio*100:.0f}% of unknowns)"
        )

        return unknown_ratio > 0.10 and confusion_ratio > 0.15
    ...
    def _correct_with_pynini(self, text: str) -> str:
        ...
        Returns:
            Corrected text
        """
        # Build OCR correction FST with error handling
        try:
            ...
            for word in words:
                ...
                if clean and clean.lower() not in self.dictionary:
                    try:
                        ...
                        self.logger.debug(f"Pynini FST correction failed for '{clean}': {e}")
            ...
            self.logger.warning(f"Pynini not available: {e}")
            return self._correct_with_edit_distance(text)
        except Exception as e:
            self.logger.warning(f"Pynini FST correction error: {e}")
            return text
    ...
    def _correct_with_edit_distance(self, text: str) -> str:
        ...
        self.logger.info(f"OCR scanning {total:,} words against dictionary...")
        for word in words:
            # Progress every 20000 words
            if len(corrected_words) % 20000 == 0 and len(corrected_words) > 0:
                pct = len(corrected_words) / total * 100
                self.logger.info(f"  OCR progress: {len(corrected_words):,}/{total:,} ({pct:.0f}%)")
            clean = re.sub(r'[^\w]', '', word)
